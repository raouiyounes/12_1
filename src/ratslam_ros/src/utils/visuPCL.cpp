...
int test( string  pcdFile)
{
	bool justVisualize=true;
...
*/
	...
	if (justVisualize)
	{
		...
		try
		{
			io::loadPCDFile<PointXYZRGBA>(pcdFile, *cloudptr);
		}
		catch (PCLException e1)
		{
			try
			{
				...
				io::loadPCDFile<PointXYZ>(pcdFile, *fallbackCloud);
			}
