...
void image_callback(sensor_msgs::ImageConstPtr image)
{
	...
	cout<<odom_output.twist.twist.linear.x;
	try
	    {
	      cv_ptr = cv_bridge::toCvCopy(image, sensor_msgs::image_encodings::BGR8);

	      cv::imshow("jjjjjjjjj", cv_ptr->image);
	          cv::waitKey(3);	    }
...
int main(int argc, char * argv[])
{
  ...
  image_transport::Subscriber sub = it.subscribe(topic_root + "/irat_red/camera/image", 1, image_callback);
